...
scrutoscope.dev stores only ciphertext. Encryption/decryption is client-side (AES-256-GCM via Web Crypto API). Decryption key lives in URL fragment (#key) — never sent to server. Scrutoscope (the org) cannot view shared reports.
...
"Send to Agent" always revokes any existing Scrutoscope Application Password (matched by `app_id`) and creates a fresh one. No option to keep old passwords alive — only one credential is ever valid at a time. Plaintext is returned once from core and copied to clipboard; we never store it.

### D25b: app_id for lifecycle management
All Scrutoscope-created Application Passwords use a fixed `app_id` UUID. This lets us: (1) find/revoke our passwords without touching others, (2) clean up on plugin deactivation, (3) detect if a valid password already exists. The `app_id` is the handle for all password lifecycle operations.
...
## D46: Outbound HTTP URLs reduced to scheme + host (1.1.0)
Stored/shared `http_calls[].url` keeps only scheme + host — paths carry secret tokens (Slack `services/T../B../<token>`, Telegram `bot<token>/...`). Reduction runs at write (Storage), on read (`get_profile` re-applies the idempotent `Storage::sanitize_profile`), and at the output boundary (`Sanitizer`), so legacy rows can't leak a path through a share/export. Same defense-in-depth shape as the SQL reducer. The destination host is enough for attribution. (See the matching INVARIANT.)

## D47: HTTP "blocking" is measured from the request flag, not duration (1.1.0)
The timeline distinguishes blocking vs. fire-and-forget async calls using the captured `blocking` request arg (recorded via `http_api_debug`, which fires for non-blocking calls too — `http_response` does not). The earlier heuristic ("blocking" = duration > 20% of the request) measured *dominance*, not blocking, and was wrong on both ends since WP genuinely supports non-blocking + parallel HTTP. No duration factor decides blocking; bar width shows duration, the flag shows blocking.

## D48: Lightweight capture mode for production-safe sampling (1.2.0)
An opt-in capture mode that records source/attribution totals but skips the timeline and the per-callback trace — together ~95% of stored profile size (a 3.9 MB profile → ~200 KB). The full "who owns the time" breakdown is kept; only the heavy per-call detail is dropped. This makes always-on background sampling safe on busy production sites, which removes the "not recommended for production" asterisk. It is *more* aligned with the output boundary, not less — aggregate without detailed inspection. The Timeline/Trace tabs (and the shared-report viewer) note when a capture was lightweight.

## D49: Public API contract for external integrations (unreleased)
Minn Admin v0.22.0 (Austin Ginder) shipped a deep integration: profile lists, detail views via our REST API, a status card reading our options, cron inventory via `Diagnostics\Cron::collect()`, and on-demand cron profiling via `Profiler::profile_cron_hook()` gated on `method_exists`. That makes our internal names a de facto public contract. Locked: the surface listed in INVARIANTS.md → Public API Surface is stable; changes require a deprecation cycle, and every contract point carries an inline "Public API" docblock note so future agents see it at the edit site. Because Minn had to read the profiles table directly to list individual captures (our REST only grouped by route), we added `GET /scrutoscope/v1/profiles` so integrators get a supported list path and direct table reads can go away.
