...
| 5 | **Profile detail tabs** *(manual)* | open a profile from a route's table | detail shows tabs **Timeline / Breakdown / Sources / Queries / Metadata** (D19, Timeline default); each switches without error. **Manual, not scripted:** opening a profile is interaction-sensitive and large legacy (pre-cap) profiles render slowly — verify by hand. |
