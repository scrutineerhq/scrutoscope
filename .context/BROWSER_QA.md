...
>
> The relay viewer has its own automated Playwright suite in the `scrutoscope-relay` repo — this runbook is only the plugin dashboard.
...
```sh
export SCRUTOSCOPE_BASE_URL="http://localhost:8888"   # or the live host
...
wp-env run cli wp option update scrutoscope_background_profiling 1
wp-env run cli wp option update scrutoscope_sample_rate 100
for u in / /?p=1 /sample-page/; do curl -s "http://localhost:8888$u" >/dev/null; done
wp-env run cli wp option update scrutoscope_sample_rate 10
```
...
|---|---|---|---|
| 1 | **Login + dashboard loads** | `/wp-admin/tools.php?page=scrutoscope` | HTTP < 400; **no** `Fatal error` / `Warning:` / `Notice:` in the body |
| 2 | **Home cards** | — | three cards: **Capture Profile** (`#scrutoscope-home-capture`), **View Profiles** (`#scrutoscope-home-profiles`), **Settings** (`#scrutoscope-home-settings`) |
| 3 | **Routes view** | click `#scrutoscope-home-profiles` | a routes table renders `tbody tr` rows; top tabs **Routes / History / Cron / API** (`.scrutoscope-top-tab`); filter `#scrutoscope-route-filter` + search `#scrutoscope-route-search` present |
| 4 | **Route drill-down** | click a route row | route detail (`#scrutoscope-route-detail`) shows that route's profile table (`#scrutoscope-profile-table`, sortable cols); `← Back to routes` (`#scrutoscope-back-to-list`) returns |
| 4b | **Regression verdict banner** | (loads on drill-down) | `#scrutoscope-route-regression` becomes visible with a `verdict-*` class (Likely Regression / Difference observed / Within noise / insufficient-data-shows-just-the-message). Routes with < ~20 profiles show insufficient-data — that's expected |
...
| 7 | **Settings + save** | click `#scrutoscope-home-settings` | sections present: Profiling, Background Measurement, Query Profiling, Storage, Profile Retention, Network, Client IP Detection. Toggling `#scrutoscope-qp-toggle` / `#scrutoscope-bg-toggle` persists (AJAX success), no error |
