...
class Storage {
	...
	public static function save_profile( $session_id, $profile_data, $profile_type = 'session', $response_status = 0 ) {
		...
		$route_key   = StorageRouteAggregates::normalize_route_key( $method, $url, $ajax_action );
		...
		try {
			...
			StorageRouteAggregates::record_route_stat( $fingerprint, $captured_at, (int) $dur_ns );
		} catch ( \Throwable $e ) {
