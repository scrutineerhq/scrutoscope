...
const DASH = '/wp-admin/tools.php?page=scrutoscope';
...
test('home loads cleanly with the three cards', async ({ page }) => {
  ...
  await expect(page.locator('#scrutoscope-home-capture')).toBeVisible();
  await expect(page.locator('#scrutoscope-home-profiles')).toBeVisible();
  await expect(page.locator('#scrutoscope-home-settings')).toBeVisible();
});

test('routes view renders rows and the top tabs', async ({ page }) => {
  await page.goto(DASH, { waitUntil: 'networkidle' });
  await page.click('#scrutoscope-home-profiles');
  await expect(page.locator('tbody tr').first()).toBeVisible({ timeout: 15000 });
  await expect(page.locator('#scrutoscope-route-filter')).toBeVisible();
  await expect(page.locator('#scrutoscope-route-search')).toBeVisible();
...
test('route drill-down opens the profile table and Back returns', async ({ page }) => {
  ...
  await expect(page.locator('#scrutoscope-back-to-list').first()).toBeVisible({ timeout: 15000 });
  await page.locator('#scrutoscope-back-to-list').first().click();
  await expect(page.locator('#scrutoscope-route-filter')).toBeVisible({ timeout: 15000 });
});
...
test('profile detail opens and the timeline tab renders without errors', async ({ page }) => {
  ...
  await page.locator('#scrutoscope-route-detail a:has-text("View"), #scrutoscope-profile-table a:has-text("View")')
    .first().click({ timeout: 15000 });
  await expect(page.locator('.scrutoscope-pin-toolbar, .scrutoscope-tabs').first()).toBeVisible({ timeout: 25000 });
  // Timeline tab is lazy-loaded; it must render (the page-error fixture guards
  // against the asset-src crash class).
  await page.locator('.scrutoscope-tab[data-tab="timeline"]').first().click();
  await page.waitForTimeout(2500);
  await expect(page.locator('.scrutoscope-tab[data-tab="timeline"]').first()).toHaveClass(/active/);
});
...
test('route detail shows a regression verdict banner', async ({ page }) => {
  ...
  const banner = page.locator('#scrutoscope-route-regression');
...
test('settings render and a toggle saves', async ({ page }) => {
  await page.goto(DASH, { waitUntil: 'networkidle' });
  await page.click('#scrutoscope-home-settings');
  await expect(page.locator('#scrutoscope-settings-view')).toBeVisible({ timeout: 15000 });

  // The functional controls are present.
  await expect(page.locator('#scrutoscope-bg-toggle')).toBeVisible();
  await expect(page.locator('#scrutoscope-retention-select')).toBeVisible();
  ...
  const bg = page.locator('#scrutoscope-bg-toggle');
