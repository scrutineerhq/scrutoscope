...
class ReportRegressionSummaryTest extends TestCase {
	...
	private function profiles( $count, $duration_ms ) {
		...
		for ( $i = 0; $i < $count; $i++ ) {
			$out[] = array(
				'request' => array( 'route_class' => 'single', 'user_role' => 'anonymous' ),
				'summary' => array( 'duration_ns' => (int) ( $duration_ms * 1000000 ) ),
			);
		}
		return $out;
	}
	...
	public function test_shape_has_all_keys() {
		$data = Report::regression_summary( array( 'baseline' => $this->profiles( 6, 200 ), 'current' => $this->profiles( 6, 200 ) ) );
